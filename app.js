...
io.sockets.on('connection', (socket) => { // Socket開始
  ...
  socket.on('connected', (usr) => {
    ...
    const socketsname = String(usr.sendmsg).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/\r?\n/g, '').replace(/\s+/g, '');
  ...
  socket.on('C_to_S_message', (data) => {
    const msg = String(data.sendmsg).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
